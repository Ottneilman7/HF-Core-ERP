...
export default function CustomersPage() {
  ...
  // Number.isFinite atrapa NaN (a diferencia de ?? 0, que solo cubre null/undefined) —
  // necesario tras el bug real de saldo corrupto en DAKA (ver ADR-009, adenda).
  const totalBalance = customers.reduce((sum, c) => sum + (Number.isFinite(c.balance) ? c.balance : 0), 0);

  const groupsPresent = new Set(customers.map((c) => groupFor(c.businessName[0])));
              ...
              <Card key={customer.id}>
                {editingId === customer.id ? (
                  <div>
                    ...
                    <FormInput
                      label="Saldo (ajuste manual — usar solo para corregir errores)"
                      type="number"
                      step="0.01"
                      value={Number.isFinite(editForm.balance) ? editForm.balance : 0}
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        ...
                        {customer.tradeName && customer.tradeName !== customer.businessName && (
                          <p style={{ color: colors.textMuted, margin: "-8px 0 8px" }}>({customer.tradeName})</p>
                        )}
                      </div>
                      <button onClick={() => startEdit(customer)} style={{ background: "transparent", border: `1px solid ${colors.border}`, color: colors.text, borderRadius: "8px", padding: "4px 10px", fontSize: "12px", cursor: "pointer" }}>
                        Editar
                      </button>
                    </div>
                    <p><strong>Código:</strong> {customer.code}</p>
                    <p><strong>Contacto:</strong> {customer.contactName}</p>
                    <p><strong>Ciudad:</strong> {customer.city}</p>
                    <p><strong>Tipo:</strong> {customer.customerType}</p>
                    {customer.taxId && <p><strong>RIF/CI:</strong> {customer.taxId}</p>}
                    {customer.retentionAgentType && customer.retentionAgentType !== "none" && (
                      <p><strong>Agente de retención:</strong> {customer.retentionAgentType === "agent_75" ? "75%" : "100%"}</p>
                    )}
                    <p><strong>Saldo:</strong> ${(Number.isFinite(customer.balance) ? customer.balance : 0).toFixed(2)}</p>
                  </>
                )}
              </Card>
            ))}
